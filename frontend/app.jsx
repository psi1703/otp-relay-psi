...
const API = {
  ...
  claimOtp(token) {
    return this.json("/claim-otp", {
      method: "POST",
      body: JSON.stringify({ token }),
    });
  },
  claimStatus(token) {
    return this.json(`/claim-status/${encodeURIComponent(token)}`);
  },
  deleteClaim(token) {
    return this.json(`/claim-otp/${encodeURIComponent(token)}`, {
      method: "DELETE",
    });
  },
  saveWizard(payload) {
    return this.json("/wizard/progress", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  },
  getWizard(token) {
    return this.json(`/wizard/progress/${encodeURIComponent(token)}`);
  },
  adminAuthStatus() {
    return this.json("/admin/auth/status");
  },
  adminAuthSetup(credential, current) {
    return this.json("/admin/auth/setup", {
      method: "POST",
      body: JSON.stringify({ credential, current }),
    });
  },
  adminAuthLogin(credential) {
    return this.json("/admin/auth/login", {
      method: "POST",
      body: JSON.stringify({ credential }),
    });
  },
  adminAuthLogout(session) {
    return this.json("/admin/auth/logout", {
      method: "POST",
      headers: { "X-Admin-Session": session },
    });
  },
  adminWizard(session) {
    return this.json("/admin/wizard", {
      headers: { "X-Admin-Session": session },
    });
  },
  adminQueue(session) {
    return this.json("/admin/queue", {
      headers: session ? { "X-Admin-Session": session } : {},
    });
  },
  adminUsers(session) {
  ...
  adminLog(session) {
    return this.json("/admin/log?limit=500", {
      headers: session ? { "X-Admin-Session": session } : {},
    });
  },
  adminConfig(session) {
  ...
  saveAdminConfig(session, admin_tokens) {
    return this.json("/admin/config", {
      method: "POST",
      headers: { "X-Admin-Session": session },
      body: JSON.stringify({ admin_tokens }),
    });
  },
  notifyAdminTask(payload) {
    return this.json("/api/onboard/notify", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  },
...
const STEPS = [
  {
    id: "form",
    title: "Submit the RTA Access Form",
    owner: "user",
    icon: "📝",
    time: "10 min",
    summary:
      "Fill the official access form and send it to Jathin with your signature and required attachments.",
    details: [
      {
        type: "info",
        text: "Complete the official RTA Access Request form and attach the supporting documents.",
      },
      {
        type: "list",
        title: "What to do",
      ...
      },
      {
        ...
        items: [
          {
            label: "RTA Access Requests (SharePoint)",
            href: "https://initse.sharepoint.com/:f:/r/sites/RTAinternal/Shared%20Documents/RTA%20Documents/PAM/RTA%20Access%20Requests",
          },
          {
            label: "Email template to Jathin",
            href: "mailto:jprakash@initse.com?cc=cschilling@initse.com;adarwich@initse.com&subject=RTA-NG-2024%20Request%20RTA%20User%20Account",
          },
        ],
      },
    ],
  },
  {
    ...
    summary:
      "Jathin applies for your RTA account in the RTA system and shares the IITS username with you.",
    details: [
      {
        type: "info",
        text: "Jathin applies for your RTA account and notifies you once the IITS username is ready.",
      },
    ],
  },
  {
    id: "adm_request",
    title: "Request ADM Account & PAM Onboarding",
    owner: "admin",
    icon: "🔧",
    time: "3–7 days",
    gate: ["form"],
  ...
  {
    id: "save_iits",
    title: "Save Your IITS Username",
    owner: "user",
    icon: "👤",
    time: "2 min",
    gate: ["account_creation"],
    summary:
      "Once Jathin sends you the IITS username, save it here for later use.",
    details: [
      {
        type: "info",
        text: "You will use the IITS account for VPN login, password resets, and OTP-related RTA access.",
      },
    ],
  },
  {
    id: "save_adm",
    title: "Save Your ADM Username",
    owner: "user",
    icon: "🗂️",
    time: "2 min",
    gate: ["adm_request"],
    summary:
      "Once Amer confirms the ADM account, save it here for PAM and server access workflows.",
    details: [
      {
        type: "info",
        text: "ADM is used for PAM and privileged access workflows inside the RTA environment.",
      },
    ],
  },
  {
    id: "password_reset",
    title: "Reset RTA Passwords",
    owner: "user",
    icon: "🔐",
    time: "15 min",
    gate: ["save_iits", "save_adm"],
    expiryKey: "iits_pw_date",
    secondExpiryKey: "adm_pw_date",
    summary:
      "Reset your IITS and ADM passwords, then record the reset dates so the 90-day countdown is visible.",
    details: [
      {
        type: "info",
        text: "The password reset link only works inside UAE. If you are outside UAE, use the Dubai terminal server first.",
      },
      {
        type: "list",
        title: "OTP relay sequence",
        items: [
      ...
      {
        type: "list",
        title: "Terminal server when outside UAE",
        items: [
      ...
      {
        type: "warn",
        text: "Passwords expire every 90 days. No automatic reminder is sent by RTA.",
      },
    ],
  },
  {
    ...
    gate: ["password_reset"],
    summary:
      "Register Oracle Authenticator for TOTP and verify it works for both IITS and ADM flows.",
    details: [
      {
        type: "list",
        title: "What to do",
  ...
  {
    id: "vpn_request",
    title: "Request VPN / PAM / SFTP Access",
    owner: "user",
    icon: "🌐",
    time: "20 min",
    gate: ["oracle_auth"],
    expiryKey: "vpn_date",
    summary:
      "Submit the VPN request in the RTA Automation portal and include the needed applications and risk IDs.",
    details: [
      {
        type: "kv",
      ...
      {
        type: "warn",
        text: "VPN access also expires every 90 days and must be renewed manually.",
      },
    ],
  },
  {
    id: "email_support",
    title: "Email RTA IT Support",
    owner: "user",
    icon: "✉️",
    time: "5 min",
    gate: ["vpn_request"],
    summary:
      "After the request is submitted, email RTA IT support to grant the access and reference the request details.",
    details: [
      {
        type: "info",
        text: "Use the RTA Automation Portal > IT Help Desk if you need to raise a support ticket or chase approvals.",
      },
    ],
  },
  {
    id: "install_vpn",
    title: "Install Ivanti and Test Access",
    owner: "user",
    icon: "💻",
    time: "15 min",
    gate: ["email_support"],
    summary:
      "Install Ivanti Secure Access Client, add the RTA VPN connection, and test VPN/PAM/SFTP access.",
    details: [
      {
        type: "kv",
        title: "Connection",
      ...
      {
        type: "info",
        text: "For test servers: connect VPN → RDP to Jump Server → then connect to the target server.",
      },
    ],
  },
];
...
function getVisibleDone(user, step) {
  if (step.owner === "admin")
    return (user.adminCompleted || []).includes(step.id);
...
function nextUserStep(user) {
  return STEPS.find(
    (step) =>
      step.owner === "user" &&
      isUnlocked(user, step) &&
      !getVisibleDone(user, step),
  );
}
...
function Logo() {
  return (
    <svg
      ...
      viewBox="0 0 160 44"
      height="36"
      aria-label="INIT — The Future of Mobility"
    >
...
function App() {
  ...
  const [otp, setOtp] = useState({
    ...
    queueDepth: 0,
    otpValue: "———",
    activeRemaining: CONFIG.CLAIM_EXPIRY_SEC,
    otpRemaining: CONFIG.OTP_DISPLAY_SEC,
  });
      ...
      try {
        ...
        if (data.status === "delivered" && data.otp) {
          setOtp((s) => ({
            ...
            panel: "otp",
            otpValue: data.otp,
            otpRemaining: data.expires_in || CONFIG.OTP_DISPLAY_SEC,
          }));
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      setOtp((s) => {
        if (s.panel === "active")
          return { ...s, activeRemaining: Math.max(0, s.activeRemaining - 1) };
        if (s.panel === "otp")
          return { ...s, otpRemaining: Math.max(0, s.otpRemaining - 1) };
        return s;
      });
    }, 1000);
  ...
  const pendingAdminTasks = useMemo(() => {
    ...
    admin.data.users.forEach((user) => {
      STEPS.filter((s) => s.owner === "admin").forEach((step) => {
        if (isUnlocked(user, step) && !getVisibleDone(user, step))
          tasks.push({ user, step });
      });
    });
    return tasks;
  }, [admin.data]);
  ...
  async function saveWizard(patch) {
    const next = {
      ...wizardUser,
      ...patch,
      token: (patch.token ?? wizardUser.token).trim().toUpperCase(),
    };
    setWizardUser(next);
    ...
    try {
      ...
      setTimeout(
        () =>
          setWizardStatus((s) =>
            s.message === "Saved to server" ? { ...s, message: "" } : s,
          ),
        1500,
      );
    } catch (e) {
  ...
  async function toggleStep(step) {
    ...
    if (turningOn) list.add(step.id);
    else list.delete(step.id);
  ...
  async function claimOtp() {
    ...
    try {
      ...
      setOtp((s) => ({
        ...s,
        panel: "error",
        message: e.message || "Could not claim slot",
      }));
    }
  }
  ...
  async function retryOtp() {
    try {
      if (otp.token) await API.deleteClaim(otp.token);
    } catch {}
    const token = otp.token;
    setOtp((s) => ({
      ...s,
      tokenChars: token
        ? token.split("").concat([""]).slice(0, 3)
        : ["", "", ""],
    }));
    if (token) {
      try {
        ...
        setOtp((s) => ({
          ...s,
          panel: "error",
          message: e.message || "Could not re-queue",
        }));
      }
    }
  }
  ...
  async function doAdminAuth() {
    ...
    try {
      if (admin.mode === "setup") {
        if (!admin.credential || admin.credential !== admin.confirm)
          throw new Error("Credentials do not match");
        const data = await API.adminAuthSetup(
          admin.credential,
  ...
  async function loadAdminData(session = admin.session) {
    ...
    try {
      const [wizard, queue, users, log, config] = await Promise.all([
        ...
        API.adminConfig(session).catch(() => ({
          admin_tokens: ["JA", "AM", "CS"],
        })),
      ]);
      setAdmin((s) => ({
        ...s,
        ...
        configTokens: (config.admin_tokens || []).join(", "),
        loading: false,
      }));
    } catch (e) {
      setAdmin((s) => ({ ...s, error: e.message, loading: false }));
  ...
  async function toggleAdminStep(token, stepId) {
    ...
    try {
      await API.notifyAdminTask({
        token,
        step_id: stepId,
        action: completed.has(stepId) ? "done" : "undone",
      });
    } catch {}
    await loadAdminData();
  }
  ...
  async function logoutAdmin() {
    try {
      await API.adminAuthLogout(admin.session);
    } catch {}
    ...
    <div className="side-stack">
      <div className="card side-card">
        ...
        <div className="notes-list">
          <div className="small">
            Claim the OTP slot first, then trigger the RTA OTP only when the
            portal tells you to.
          </div>
          <div className="small">
            The wizard is shared and server-backed, so credentials and reminder
            dates follow the user token across devices.
          </div>
          <div className="small">
            Admins can monitor onboarding progress and complete the admin-owned
            steps.
          </div>
        </div>
      </div>
      <div className="card side-card">
        ...
        <div className="quick-links">
          <a className="quick-link" href="https://srvotp26.init-db.lan">
            <span>OTP Relay</span>
            <small>LAN</small>
          </a>
          <a className="quick-link" href="https://direct.rta.ae">
            <span>RTA Automation Portal</span>
            <small>Portal</small>
          </a>
          <a className="quick-link" href="https://srvterminal.init-db.lan">
            <span>Terminal Server</span>
            <small>UAE-only workaround</small>
          </a>
          <a className="quick-link" href="https://ettisal.rta.ae/vendors">
            <span>Ivanti VPN</span>
            <small>ettisal.rta.ae</small>
          </a>
        </div>
      </div>
    </div>
    ...
    <>
      <header className="topbar">
        <div className="topbar-left">
          <Logo />
          <span className="topbar-title">OTP Portal</span>
        </div>
        <div className="topbar-right">
          {["otp", "wizard", "help", "admin"].map((v) => (
            <span
              ...
              onClick={() => {
                setView(v);
                if (v === "admin" && admin.session && !admin.data)
                  loadAdminData();
              }}
            >
              {v === "otp"
      ...
      <main className="app-shell">
        ...
        {view === "help" && (
          <HelpView faqOpen={faqOpen} setFaqOpen={setFaqOpen} />
        )}
        {view === "admin" && (
          <AdminView
            admin={admin}
            setAdmin={setAdmin}
            doAdminAuth={doAdminAuth}
            loadAdminData={loadAdminData}
            toggleAdminStep={toggleAdminStep}
            pendingAdminTasks={pendingAdminTasks}
...
function OtpView({ otp, setOtp, claimOtp, retryOtp, resetClaim, sidebar }) {
  ...
  const ringValue =
    otp.panel === "otp" ? otp.otpRemaining : otp.activeRemaining;
  const ringTotal =
    otp.panel === "otp" ? CONFIG.OTP_DISPLAY_SEC : CONFIG.CLAIM_EXPIRY_SEC;
      ...
      <div>
        {otp.panel === "claim" && (
          <div className="card claim-card">
            ...
            <div className="sub">
              Enter your personal token and claim your slot. The OTP code will
              appear right here — no email needed.
            </div>
            ...
            <div className="token-hint">
              2 or 3 characters · letters and digits only
            </div>
            <button
            ...
            <div className="footer-note">
              Never share your OTP with anyone — not even IT. Especially not IT.
            </div>
          </div>
        )}
          ...
          <div className="card status-card">
            {otp.panel === "active" && (
              <>
                ...
                <div className="sub">
                  Open the platform and request the SMS code. It will appear on
                  this screen within seconds.
                </div>
              </>
            )}
            {otp.panel === "waiting" && (
              <>
                <span className="queue-badge warn">
                  Position #{otp.position} in queue
                </span>
                <h2 className="status-title">Hang tight — almost your turn</h2>
                <div className="sub">
                  Someone is ahead of you. Do not trigger your OTP yet. Wait
                  until this page tells you to.
                </div>
              </>
            )}
            {otp.panel === "otp" && (
              <>
                ...
                <div className="sub">
                  Use it now — it expires on the platform, not just here.
                </div>
                <div className="otp-box">
                  <div className="otp-label">One-Time Password</div>
                  <div className="otp-code">{otp.otpValue}</div>
                </div>
              </>
            )}
            {otp.panel === "expired" && (
              <>
                ...
                <h2 className="status-title">
                  Slot reclaimed — no hard feelings
                </h2>
                <div className="sub">
                  90 seconds passed without an OTP arriving. Claim your slot
                  first, then trigger the OTP in that order.
                </div>
                <button className="btn btn-primary" onClick={resetClaim}>
                  Try again
                </button>
              </>
            )}
            {otp.panel === "error" && (
              <>
                ...
                <button className="btn btn-danger" onClick={resetClaim}>
                  Try again
                </button>
              </>
            )}
            ...
            {(otp.panel === "active" || otp.panel === "otp") && (
              <div className="ring-wrap">
                <svg width="116" height="116" viewBox="0 0 116 116">
                  ...
                  <circle
                    className={`ring-fill ${otp.panel === "otp" ? "success" : ""} ${otp.panel === "otp" && otp.otpRemaining < 60 ? "warn" : ""}`}
                    cx="58"
                    cy="58"
                    r="42"
                    strokeDasharray={CONFIG.RING_CIRCUMFERENCE}
            ...
            {otp.panel === "waiting" && (
              <div>
                ...
                >
                  <span className="mono">Position {otp.position}</span> ·{" "}
                  <span className="mono">Est. wait {otp.waitEstimate}s</span>
                </div>
                <div className="queue-room">
                  {Array.from(
                    { length: otp.queueDepth || otp.position },
                    (_, idx) => idx + 1,
                  ).map((n) => (
                    <div
                      key={n}
                      className={`queue-row ${n === 1 ? "active" : ""} ${n === otp.position ? "you" : ""}`}
                    >
                      <div className={`dot ${n === 1 ? "active" : ""}`}>
                        {n}
                      </div>
            ...
            {(otp.panel === "active" || otp.panel === "waiting") && (
              <div className="status-list">
                <div className="status-step">
                  ...
                  <div>Slot claimed successfully</div>
                </div>
                <div className="status-step">
                  <div
                    className={`dot ${otp.panel === "active" ? "active" : ""}`}
                  >
                    2
                  </div>
                  <div>
                    {otp.panel === "active"
                      ? "Trigger the OTP on the RTA platform now"
                      : "Wait for the green light before touching the RTA page"}
                  </div>
                </div>
                <div className="status-step">
                  <div className="dot">3</div>
                  <div>The OTP appears here automatically</div>
                </div>
              </div>
            )}
    ...
    <div className="wide-layout">
      <div className="card main-panel">
        <div className="hero-row">
          <div>
            ...
            <div className="sub">
              Manager-style layout, but in the INIT light theme and official RS
              colors. Your token record is server-backed so reminders and
              progress persist across devices.
            </div>
          </div>
          <div className="hero-meta">
            ...
            {nextStep && (
              <span className="pill warn">Up next: {nextStep.title}</span>
            )}
          </div>
        </div>
        ...
        <div className="card progress-card" style={{ boxShadow: "none" }}>
          ...
          >
            <div>
              ...
              <div className="small">
                User and admin-owned steps are counted together.
              </div>
            </div>
            {wizardStatus.message && (
              <div
                className={
                  wizardStatus.message.includes("failed")
                    ? "error-box"
                    : "success-box"
                }
              >
        ...
        <div className="step-grid">
          {STEPS.map((step) => {
            ...
            return (
              <div
                key={step.id}
                className={`step-card ${step.owner === "admin" ? "admin" : ""} ${isNext ? "next" : ""} ${!unlocked ? "locked" : ""}`}
              >
                <div className={`rail ${done ? "done" : isNext ? "next" : ""}`}>
                  {done ? "✓" : step.icon}
                </div>
                <div>
                  <div
                  ...
                  >
                    ...
                    <div className="step-tags">
                      <span
                        className={`pill ${step.owner === "admin" ? "warn" : "primary"}`}
                      >
                        {step.owner === "admin" ? "Admin" : "You"}
                      </span>
                  ...
                  <div className="step-actions">
                    {step.owner === "user" ? (
                      ...
                      <span className={`pill ${done ? "success" : "warn"}`}>
                        {done ? "Completed by admin" : "Waiting for admin"}
                      </span>
                    )}
                    <button
                    ...
                    {isNext && !done && (
                      <span className="pill primary">← Up next</span>
                    )}
                  </div>
      ...
      <div className="side-panel">
        <div className="card side-card">
          ...
          <div className="form-grid">
            <div className="field">
              ...
              <input
                value={user.token || ""}
                onChange={(e) => saveWizard({ token: e.target.value })}
                placeholder="2–3 chars"
              />
            </div>
            <div className="field">
              ...
              <input
                ...
                onChange={(e) => saveWizard({ display_name: e.target.value })}
                placeholder="e.g. Sara"
              />
            </div>
            <div className="field">
              <label>IITS username</label>
              <input
                value={user.iits_username || ""}
                onChange={(e) => saveWizard({ iits_username: e.target.value })}
                placeholder="IITS_…"
              />
            </div>
            <div className="field">
              ...
              <input
                value={user.adm_username || ""}
                onChange={(e) => saveWizard({ adm_username: e.target.value })}
                placeholder="ADM_…"
              />
            </div>
          </div>
        ...
        <div className="card side-card">
          ...
          <CountdownEntry
            label="IITS Password"
            date={user.iits_pw_date}
            onDateChange={(d) => saveWizard({ iits_pw_date: d })}
            onReset={() =>
              saveWizard({ iits_pw_date: new Date().toISOString() })
            }
          />
          <CountdownEntry
            label="ADM Password"
            date={user.adm_pw_date}
            onDateChange={(d) => saveWizard({ adm_pw_date: d })}
            onReset={() =>
              saveWizard({ adm_pw_date: new Date().toISOString() })
            }
          />
          <div className="small" style={{ marginTop: 10 }}>
            Passwords expire every 90 days. No reminders from RTA.
          </div>
        </div>
        ...
        <div className="card side-card">
          ...
          <CountdownEntry
            label="VPN Access"
            date={user.vpn_date}
            onDateChange={(d) => saveWizard({ vpn_date: d })}
            onReset={() => saveWizard({ vpn_date: new Date().toISOString() })}
          />
          <div className="small" style={{ marginTop: 10 }}>
            VPN, PAM, and SFTP access all expire after 90 days.
          </div>
        </div>
        ...
        <div className="card side-card">
          ...
          <div className="quick-links">
            <a className="quick-link" href="https://srvotp26.init-db.lan">
              <span>OTP Relay</span>
              <small>Claim first</small>
            </a>
            <a className="quick-link" href="https://direct.rta.ae">
              <span>RTA Automation Portal</span>
              <small>Main portal</small>
            </a>
            <a className="quick-link" href="https://srvterminal.init-db.lan">
              <span>Terminal Server</span>
              <small>Outside UAE</small>
            </a>
            <a className="quick-link" href="https://ettisal.rta.ae/vendors">
              <span>Ivanti VPN</span>
              <small>Install/test</small>
            </a>
          </div>
        </div>
        ...
        <div className="card side-card">
          ...
          <div className="notes-list">
            <div className="small">
              Full onboarding usually takes 2–3 weeks.
            </div>
            <div className="small">
              Test servers: VPN → Jump Server (RDP) → target.
            </div>
            <div className="small">
              File transfer: VPN → WinSCP → SFTP → target server.
            </div>
          </div>
        </div>
      </div>
        ...
        if (block.type === "info")
          return (
            <div key={idx} className="guide-block">
              <div className="inline-info">{block.text}</div>
            </div>
          );
        if (block.type === "warn")
          return (
            <div key={idx} className="guide-block">
              <div className="inline-note">{block.text}</div>
            </div>
          );
        if (block.type === "list")
          return (
            <div key={idx} className="guide-block">
              <div className="guide-label">{block.title}</div>
              <ul>
            ...
            <div key={idx} className="guide-block">
              ...
              {block.items.map((item, i) => (
                <div className="kv" key={i}>
                  <div className="kv-key">{item[0]}</div>
                  <div>{item[1]}</div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="side-entry">
      <div className="side-entry-head">
        ...
        <div className={`countdown ${countdownTone(days)}`}>
      ...
      <div className="date-row">
        <input
          ...
          onChange={(e) => onDateChange(fromDateInputValue(e.target.value))}
        />
    ...
    return (
      <div className="auth-wrap">
        <div className="card main-panel">
          ...
          <h1 className="h1">
            {admin.mode === "setup" ? "Set admin credential" : "Admin login"}
          </h1>
          <div className="sub">
            Use a password or 4-digit PIN. This is shared for portal admins.
          </div>
          <div className="form-grid" style={{ marginTop: 18 }}>
            {admin.mode === "setup" && admin.configured && (
              <div className="field">
                ...
                <input
                  ...
                  value={admin.current}
                  onChange={(e) =>
                    setAdmin((s) => ({ ...s, current: e.target.value }))
                  }
                />
              </div>
            )}
            <div className="field">
              <label>
                {admin.mode === "setup" ? "New credential" : "Credential"}
              </label>
              <input
                type="password"
                value={admin.credential}
                onChange={(e) =>
                  setAdmin((s) => ({ ...s, credential: e.target.value }))
                }
              />
            </div>
            {admin.mode === "setup" && (
              <div className="field">
                ...
                <input
                  ...
                  value={admin.confirm}
                  onChange={(e) =>
                    setAdmin((s) => ({ ...s, confirm: e.target.value }))
                  }
                />
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              <button
                ...
                onClick={() =>
                  setAdmin((s) => ({
                    ...s,
                    mode: s.mode === "setup" ? "login" : "setup",
                    error: "",
                  }))
                }
              >
                {admin.mode === "setup" ? "Use login" : "Change credential"}
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="admin-layout">
      <div className="admin-top">
        <div className="card stat-card">
          <div className="stat-label">Wizard users</div>
          <div className="stat-value">{users.length}</div>
        </div>
        <div className="card stat-card">
          <div className="stat-label">Pending admin tasks</div>
          <div className="stat-value">{pendingAdminTasks.length}</div>
        </div>
        <div className="card stat-card">
        ...
        <div className="card stat-card">
          ...
          <div className="stat-value">{admin.data?.logTotal || 0}</div>
        </div>
      </div>
        ...
        <div className="card main-panel">
          <div className="hero-row">
            <div>
              ...
              <div className="sub">
                Users, credentials, progress, and admin-owned steps in one view.
              </div>
            </div>
          ...
          {pendingAdminTasks.length > 0 && (
            <div
              className="card progress-card"
              style={{ boxShadow: "none", marginBottom: 14 }}
            >
              ...
              {pendingAdminTasks.map((t, i) => (
                <div key={i} className="admin-task-row">
                  <button
                    ...
                    onClick={() => toggleAdminStep(t.user.token, t.step.id)}
                  >
                    ☐
                  </button>
                  ...
                  <div>
                    ...
                    <div className="small">
                      {t.step.adminLabel || t.step.summary}
                    </div>
                  </div>
                </div>
              ))}
              ...
              {users
                ...
                .map((u) => {
                  const pct = Math.round(
                    (allDone(u).length / STEPS.length) * 100,
                  );
                      ...
                      <td>
                        {STEPS.filter((s) => s.owner === "admin").map((s) => {
                          ...
                          return (
                            <button
                              key={s.id}
                              className={`btn ${done ? "btn-secondary" : "btn-outline"}`}
                              style={{
                                marginRight: 6,
                                marginBottom: 6,
                                padding: "6px 10px",
                        ...
                        {pending.length === 0 && (
                          <div className="small">No pending admin steps</div>
                        )}
                      </td>
                    </tr>
                  );
        ...
        <div className="side-panel">
          <div className="card side-card">
            ...
            <div className="field">
              ...
              <input
                value={admin.configTokens}
                onChange={(e) =>
                  setAdmin((s) => ({ ...s, configTokens: e.target.value }))
                }
              />
            </div>
            <div className="small" style={{ marginTop: 10 }}>
              Seeded for Jathin, Amer, and Christian, but editable from the
              portal.
            </div>
            <button
              className="btn btn-primary"
          ...
          <div className="card side-card">
            ...
            {queue.length === 0 ? (
              <div className="small">Nobody is in the queue right now.</div>
            ) : (
              queue.map((q, i) => (
                <div className="queue-row" key={i}>
                  <div className="dot active">{q.position || i + 1}</div>
                  <div>
                    <strong>{q.token}</strong>
                    <div className="small">{q.name || q.email || ""}</div>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="card side-card">
            ...
            <div style={{ maxHeight: 360, overflow: "auto" }}>
              {log.slice(0, 15).map((entry, i) => (
                ...
                >
                  <strong>{entry.event}</strong> · {entry.token || "—"}
                  <br />
                  {entry.detail || ""}
                </div>
              ))}
